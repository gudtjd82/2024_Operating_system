...
00000060 <fmtname>:
{
  ...
  return buf;
  c5:	bb e8 0a 00 00       	mov    $0xae8,%ebx
  memset(buf+strlen(p), ' ', DIRSIZ-strlen(p));
  ...
  db:	05 e8 0a 00 00       	add    $0xae8,%eax
  e0:	52                   	push   %edx
  e1:	6a 20                	push   $0x20
  e3:	50                   	push   %eax
...
free(void *ap)
{
 ...
 8a1:	a1 f8 0a 00 00       	mov    0xaf8,%eax
{
 ...
 8e1:	5b                   	pop    %ebx
  freep = p;
 8e2:	89 15 f8 0a 00 00    	mov    %edx,0xaf8
}
 8e8:	5e                   	pop    %esi
 8e9:	5f                   	pop    %edi
 8ea:	5d                   	pop    %ebp
 ...
 919:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 91c:	89 15 f8 0a 00 00    	mov    %edx,0xaf8
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 939:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 93c:	8b 3d f8 0a 00 00    	mov    0xaf8,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 97f:	73 4f                	jae    9d0 <malloc+0xa0>
    ...
    if(p == freep)
 981:	8b 3d f8 0a 00 00    	mov    0xaf8,%edi
 ...
 9aa:	e8 f1 fe ff ff       	call   8a0 <free>
  return freep;
 9af:	8b 15 f8 0a 00 00    	mov    0xaf8,%edx
 ...
 9dc:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
 9df:	89 15 f8 0a 00 00    	mov    %edx,0xaf8
}
 ...
 9ef:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
 9f0:	c7 05 f8 0a 00 00 fc 	movl   $0xafc,0xaf8
 9f7:	0a 00 00 
    base.s.size = 0;
 9fa:	bf fc 0a 00 00       	mov    $0xafc,%edi
    base.s.ptr = freep = prevp = &base;
 9ff:	c7 05 fc 0a 00 00 fc 	movl   $0xafc,0xafc
 a06:	0a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 a09:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 a0b:	c7 05 00 0b 00 00 00 	movl   $0x0,0xb00
 a12:	00 00 00 
    if(p->s.size >= nunits){
